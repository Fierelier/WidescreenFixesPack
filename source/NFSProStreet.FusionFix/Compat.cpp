...
void Init()
{
    ...
    XtendedInput::SetPollingState = reinterpret_cast<decltype(XtendedInput::SetPollingState)>(GetProcAddress(XtendedInput::mhXtendedInput, "SetPollingState"));
...
public:
    Compat()
    {
        WFP::onInitEvent() += []()
        {
            CallbackHandler::RegisterCallback(L"NFS_XtendedInput.asi", Init);
        };
    }
} Compat;

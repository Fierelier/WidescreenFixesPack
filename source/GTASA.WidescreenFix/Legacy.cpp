...
extern "C" DWORD _EAX_Legacy = 0;
extern "C" uintptr_t SetEdgeAddr = 0x719590;
extern "C" int g_ReplaceTextShadowWithOutlineMirror = 0;
extern "C" float* g_LegacyScreenAspectRatioPtr = nullptr;
...
void ReadSettings()
{
    ...
    ReplaceTextShadowWithOutline = iniReader.ReadInteger("MISC", "ReplaceTextShadowWithOutline", 0);
    g_ReplaceTextShadowWithOutlineMirror = ReplaceTextShadowWithOutline;
}
...
void GetMemoryAddresses()
{
    ...
    g_LegacyScreenAspectRatioPtr = CDraw::pfScreenAspectRatio;
...
__attribute__((naked)) void AllowMouseMovement()
{
    __asm__(
        ...
        "mov [__EAX_Legacy], eax\n\t"
        "mov eax, dword ptr [0x8D621C]\n\t"
        "cmp eax, 0\n\t"
        "jne 1f\n\t"
        "mov eax, [__EAX_Legacy]\n\t"
        "ret\n\t"
        "1:\n\t"
        "mov eax, [__EAX_Legacy]\n\t"
        "mov dword ptr [__EAX_Legacy], 0x7453F0\n\t"
        "jmp dword ptr [__EAX_Legacy]\n\t"
        ".att_syntax prefix\n\t"
    );
}
...
__attribute__((naked)) void SetDropShadowPosition()
{
    __asm__(
        ".intel_syntax noprefix\n\t"
        "mov eax, dword ptr [esp+0x4]\n\t"
        "cmp dword ptr [_g_ReplaceTextShadowWithOutlineMirror], 2\n\t"
        "jae 1f\n\t"
        "test eax, eax\n\t"
        "jz 1f\n\t"
        "dec eax\n\t"
...
__attribute__((naked)) void CalculateAimingPoint()
{
    __asm__(
        ".intel_syntax noprefix\n\t"
        "fstp st(0)\n\t"
        "mov edx, dword ptr [_g_LegacyScreenAspectRatioPtr]\n\t"
        "fmul dword ptr [edx]\n\t"
    ...
    if (!bDontTouchFOV)
    {
        ...
        struct EmergencyVehiclesFix
        {
            void operator()(injector::reg_pack& regs)
            {
                ...
                __asm__ __volatile__ ("fdivs %0" : : "m" (f));
            }
...
{
    uint32_t pTexture = 0;
    __asm__ __volatile__ ("movl %%esi, %0" : "=r"(pTexture));
...
void CompatWarning()
{
    ...
    GetModuleFileNameA(GetModuleHandleA("wshps.asi"), buffer, sizeof(buffer));
    auto s = std::string(buffer);

    auto result = MessageBoxA(NULL, "wshps.asi is no longer required for the widescreen fix to work.\n\
     Click Yes to remove it, restart the game after.\n\
     Click No to continue, but issues may occur (not recommended).\n\
     You can remove wshps.asi manually later.", "GTASA.WidescreenFix.asi", MB_YESNO);

    switch (result)
    {
        case IDYES:
            MoveFileA(s.c_str(), std::string(s + ".deleteonnextlaunch").c_str());
            ExitProcess(0);
            break;
        default:
            break;
...
DWORD WINAPI CompatHandler(LPVOID)
{
    ...
    while (true)
    {
        if (!bSP && GetModuleHandle("SilentPatchSA.asi") != NULL)
        {
            OverwriteResolution();
            bSP = true;
        }

        if (!bWSHPS && GetModuleHandle("wshps.asi") != NULL)
        {
            CompatWarning();
            bWSHPS = true;
        }

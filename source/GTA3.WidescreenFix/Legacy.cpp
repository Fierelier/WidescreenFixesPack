...
void ReadSettings()
{
    CIniReader iniReader("GTA3.WidescreenFixLegacy.ini");
...
void FixFOV()
{
    ...
    }; injector::MakeInline<EmergencyVehiclesFix>(EmergencyVehiclesFixPattern.count(1).get(0).get<uint32_t>(-6), EmergencyVehiclesFixPattern.count(1).get(0).get<uint32_t>(6));
}

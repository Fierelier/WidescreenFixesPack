...
extern "C" uint32_t funcCCameraAvoidTheGeometryJmp = 0;
extern "C" float* g_LegacyScreenAspectRatioPtr = nullptr;
extern "C" const float g_LegacyConst2_0 = 2.0f;
extern "C" uint32_t originalColor = 0, _eax = 0;
extern "C" uint32_t* jmpAddr = nullptr;
...
void ReadSettings()
{
    ...
    fHudHeightScale = iniReader.ReadFloat("MAIN", "HudHeightScale", 0.0f); fHudHeightScale == 0.0f ? fHudHeightScale = 1.0714285f : fHudHeightScale;
...
__attribute__((naked)) void funcCCameraAvoidTheGeometryHook()
{
    __asm__(
        ".intel_syntax noprefix\n\t"
        "mov eax, dword ptr [_g_LegacyScreenAspectRatioPtr]\n\t"
        "fld dword ptr [_g_LegacyConst2_0]\n\t"
        "fld dword ptr [eax]\n\t"
        "fcomip st, st(1)\n\t"
        "fstp st(0)\n\t"
        "jb 1f\n\t"
        ...
        "push ebp\n\t"
        "sub esp, 0x120\n\t"
        "jmp dword ptr [_funcCCameraAvoidTheGeometryJmp]\n\t"
        ".att_syntax prefix\n\t"
    );
}
...
__attribute__((naked)) void GetTextOriginalColor()
{
    __asm__(
        ".intel_syntax noprefix\n\t"
        ...
        "mov dword ptr [_originalColor], eax\n\t"
        "mov eax, dword ptr [__eax]\n\t"
        "jmp dword ptr [_jmpAddr]\n\t"
        ".att_syntax prefix\n\t"
    );
}
...
void ApplyIniOptions()
{
    ...
    auto pattern = hook::pattern("DD D8 E8 ? ? ? ? 83 C4 0C 6A 00"); //0x55B113 subtitles

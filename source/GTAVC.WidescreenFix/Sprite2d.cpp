...
void __cdecl RenderOneXLUSprite_Rotate_Aspect(float x, float y, float z, float w, float h, uint8_t r, uint8_t g, uint8_t b, int16_t intensity, float rhw, uint8_t alpha)
{
    float off = GetHudOffset();
    x = g_skipXCorrection ? (x + off) : (CorrectX(x) + off);
    shRenderOneXLUSprite_Rotate_Aspect.unsafe_ccall(x, y, z, w, h, r, g, b, intensity, rhw, alpha);
}
...
public:
    Sprite2d()
    {
        WFP::onGameInitEvent() += []()
        {
            ...
            {
                auto pattern = hook::pattern("E8 ? ? ? ? 83 C4 20 80 3D ? ? ? ? ? 0F 84 ? ? ? ? 30 D2 31 FF");
...

            {
                ...
                pattern = hook::pattern("74 ? B9 ? ? ? ? E8 ? ? ? ? 8B 0D");
                injector::MakeNOP(pattern.get_first(), 2, true);

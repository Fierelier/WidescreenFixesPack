...
GameRef<tFontRenderStatePointer> FontRenderStatePointer([]() -> tFontRenderStatePointer*
{
...
constexpr size_t FontRenderStateBufSize = 1024;
uint8_t* FontRenderStateBuf = nullptr;
...
namespace CFont
{
    ...
    void __cdecl PrintOutlinedString(float x, float y, uint32_t, wchar_t* start, wchar_t* end, float spwidth)
    {
        ...
        if (dropShadowPosition != 0.0f && (Details->style == FONT_BANK || Details->style == FONT_STANDARD))
        {
            ...
            if (ReplaceTextShadowWithOutlineFix)
            {
                ...
                float outlineStrength = (ReplaceTextShadowWithOutlineFix > 1) ? 0.5f : 0.25f;
                ...
                const int offsetCount = (ReplaceTextShadowWithOutlineFix > 1) ? 8 : 4;
...
std::array<ProtectedGameRef<float>, FrontendClassCount> ResXInvRefs;
std::array<ProtectedGameRef<float>, FrontendClassCount> ResYInvRefs;

void StretchX(SafetyHookContext& ctx)
{
    float f = (CDrawFix::GetAspectRatio() / (4.0f / 3.0f));
...
public:
    Frontend()
    {
        WFP::onGameInitEvent() += []()
        {
            ...
            {
                FrontendMenuManager->m_fMapCenterX = ((1.0f / INV_DEFAULT_SCREEN_HEIGHT_MENU) * CDrawFix::GetAspectRatio()) / 2.0f;
            });
